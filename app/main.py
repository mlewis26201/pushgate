...
def tokens_page(request: Request, db: Session = Depends(get_db), admin=Depends(get_current_admin)):
    ...
    return templates.TemplateResponse("tokens.html", {"request": request, "tokens": decrypted_tokens})
...
def create_token(request: Request, db: Session = Depends(get_db), admin=Depends(get_current_admin)):
    ...
    db.add(token_obj)
    db.commit()
    return RedirectResponse(url="/pushgate/tokens", status_code=303)
...
def pushover_config_page(request: Request, db: Session = Depends(get_db), admin=Depends(get_current_admin)):
    ...
    return templates.TemplateResponse("pushover_config.html", {"request": request, "app_token": app_token, "user_key": user_key})
...
def update_pushover_config(request: Request, app_token: str = Form(...), user_key: str = Form(...), db: Session = Depends(get_db), admin=Depends(get_current_admin)):
    ...
    if config:
        ...
        db.add(config)
    db.commit()
    return RedirectResponse(url="/pushgate/pushover-config", status_code=303)
...
def logout(request: Request):
    ...
    return RedirectResponse(url="/pushgate/login", status_code=303)
